...
class SecondaryMarket extends Component {
  ...
  updateTickets = async () => {
    try {
      ...
      const saleTickets = await FestivalNFT.methods.getTicketsForSale().call({ from: initiator });
      const renderData = await Promise.all(saleTickets.map(async ticketId => {
        const { purchasePrice, sellingPrice, forSale } = await FestivalNFT.methods.getTicketDetails(ticketId).call({ from: initiator });
  ...
  updateFestivals = async () => {
    try {
      ...
      this.setState({ fests: renderData, fest: activeFests[0], marketplace: "0x565e2450346c0EB984897122e9D2a3fe58485999", festName: "crate2" });
    } catch (err) {
        ...
        <div class="row">
          <div class="container ">
            <div class="container ">
              ...
              <h5 style={{ padding: "30px 0px 0px 10px" }}>NFT Marketplace</h5>
              ...
              <h4 class="center">Purchase NFT</h4>

              <table id='requests' class="responsive-table striped" >
                <thead>
                  <tr>
                    <th key='name' class="center">NFT Name</th>
                    <th key='ticketId' class="center">NFT Id</th>
                    <th key='cost' class="center">Cost</th>
